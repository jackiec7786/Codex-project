import { PrismaClient, Parish, Role } from "@prisma/client";
...
async function main() {
  ...
  const user = await prisma.user.upsert({ where: { email: "user@grenadaboard.local" }, update: {}, create: { email: "user@grenadaboard.local", name: "Local Neighbor", passwordHash, role: Role.USER, parish: Parish.SAINT_GEORGE } });
  await prisma.user.upsert({ where: { email: "mod@grenadaboard.local" }, update: {}, create: { email: "mod@grenadaboard.local", name: "Community Mod", passwordHash, role: Role.MODERATOR, parish: Parish.SAINT_ANDREW } });
